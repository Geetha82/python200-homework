...
# Concepts Q1 Justification:
# Scenario A Best Approach: Retrieval-Augmented Generation (RAG)
# Explanation: RAG is ideal here because the team needs to query an expansive legal library of hundreds of 
# documents that are frequently updated every quarter, allowing seamless database additions without model retraining.
#
# Scenario B Best Approach: Fine-Tuning
# Explanation: Fine-tuning is the correct path since the startup wants the model to deeply learn a highly 
# nuanced, specialized, non-public brand voice style using thousands of standalone training examples.
#
# Scenario C Best Approach: Prompt Engineering (Context Injection)
# Explanation: Prompt engineering is optimal because it targets an isolated, one-off question covering a small, 
# single two-page report that easily fits straight into the active prompt window with zero system overhead.
...
# Concepts Q2 Justification:
# Why confidently wrong answers cause more harm:
# Confidently wrong answers actively mislead human operators into trusting false information without verifying it, 
# whereas an explicit declaration of uncertainty triggers immediate safety protocols and manual check cycles.
#
# Real-world situation of hallucination harm:
# In a corporate setting, if an AI confidently hallucinates incorrect financial ledger data or false regulatory compliance 
# guidelines, it can prompt executives to execute illegal transactions or file fraudulent compliance audits.
#
# How tone affects user trust patterns:
# Language models mimic the highly declarative, authoritative prose style found in expert manuals, which causes users to 
# bypass their critical thinking because the formal presentation sounds entirely expert and completely sure of itself.
...
#    The system reads raw binary files or text data sheets and parses them into plain string characters.
# 2. Split text into chunks
#    Long unformatted text blocks are cut down into uniform, smaller sections using a specific window size and overlap.
# 3. Convert text chunks into embeddings
#    Each text chunk is transformed into a high-dimensional numeric vector and stored inside a database index.
# 4. Receive the user's query
#    The end-user submits a natural language question or search phrase directly into the application interface.
# 5. Embed the user's query
#    The question string is translated into a single vector coordinate using the exact same embedding model.
# 6. Retrieve the most relevant chunks
#    The database conducts mathematical similarity calculations to pull the chunks closest to the query vector.
# 7. Inject retrieved chunks into the prompt
#    The plain text from the top-matching chunks is pasted directly into a structured prompt template.
# 8. Generate a response from the LLM
#    The language model reviews the prompt instructions and reference facts to produce a grounded answer.
