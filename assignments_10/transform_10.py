...
- Pipeline Confidence Score: {confidence_pct:.1f}%"""
        ...
        payloads.append({
            ...
            "ml_prediction": record["good_for_running"],
            "ml_confidence": record["confidence"],
    ...
    verify_response = supabase.table("weather_enriched").select("date", "ml_prediction", "ml_confidence", "llm_summary").execute()
    verify_data = verify_response.data
    
    # 1. Print total rows found
    total_rows = len(verify_data)
    print(f"Total number of rows in weather_enriched: {total_rows}")
    
    # 2. Print historical calculation aggregates
    total_good_days_all = sum(1 for row in verify_data if row["ml_prediction"] == 1)
    ...
    sample_rows = verify_data[:5]
    for idx, row in enumerate(sample_rows, start=1):
        print(f"  Sample #{idx}:")
        print(f"    date:             {row['date']}")
        print(f"    good_for_running: {row['ml_prediction']}")
...
# The LLM summaries accurately match the weather features and model predictions.
#
# * Particularly Good: Sample #4 (2023-01-03)
#   - Text: "Given the extremely low confidence score, it may be advisable to reconsider running outdoors today..."
#   - Why: It perfectly followed the hedging instruction by picking up on the tiny 0.0067 confidence score and shifting to a cautious tone.
#
# * Weaker Summary: Sample #2 (2023-01-01)
#   - Text: "Avoid running today due to extremely unfavorable weather conditions."
#   - Why: It is too vague and does not explain which specific weather features (rain, wind, heat) made the day bad.
#   - Cause: This happens because of a low model temperature (0.3) combined with a strict one-sentence limit, forcing gpt-4o-mini to write broad generalizations to save token space.
# ==============================================================================
